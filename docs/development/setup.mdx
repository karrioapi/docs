...
(Windows user may prefer to use the [Windows Terminal](https://aka.ms/windowsterminal)
but the Command Prompt will also work).
...
*This setup is ideal for Linux and macOS*
...
*This setup is ideal for Windows*
...
```shell
docker-compose run --rm api karrio migrate
docker-compose run --rm api karrio collectstatic --noinput
```

- Finally, create yourself an admin account:

```shell
docker-compose run --rm api karrio createsuperuser
```

- Start the server:

```shell
docker-compose run --rm api karrio runserver 0.0.0.0:5002
```
