...
```terminal title="Terminal"
source ./bin/setup-server-env --light
```
...
Any file changes will automatically reload the server.

Once the docker compose environment is up, you can still use all the scripts using `docker exec`.

e.g: `docker exec bash -c './bin/run-server-test'`

e.g: `docker exec bash -c 'python -m unittest discover -v -f sdk/extensions/dhl_express/tests'`
