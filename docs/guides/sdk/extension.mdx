...
1. Create a karrio extension package
  1. Create a new karrio extension by copying [.templates/carrier](https://github.com/PurplShip/purplship/tree/main/.templates/carrier)
...
:::info `purplship.mappers.[carrier_name]`"
  ...
  > on runtime, karrio retrieves all mappers by going trought the `purplship.mappers` modules
:::

:::info `purplship.providers.[carrier_name]`"
  This is where the mapping between karrio Unified API data is mapped on the carrier data type corresponding requests
:::

#### extension signature

The Mapper is the cornerstone ofkarrio 's abstraction. A `Metadata` declared at `purplship.mappers.[carrier_name].__init__`
